defmodule DoIt.Completion do
  ...
  def parse_completion_context(main_module, args) do
    ...
    {completed_args, current_token} = case args do
      [] -> {[], ""}
            ...
            case Introspection.resolve_command_path(main_module, path) do
              ...
              command_module ->
                ...
                if current_token == "" do
                  ...
                  subcommands = Introspection.get_subcommands(command_module)
                  options = Introspection.get_command_option_flags(command_module)
                  if length(subcommands) > 0 || length(options) > 1 do  # > 1 because --help is always present
  ...
  defp parse_completion_context(_main_module, [], _current_token, command_path, _arg_index, waiting_for_option) do
    # We have a waiting option and current_token is its value
    {:option_value, waiting_for_option, Enum.reverse(command_path)}
  end

  defp parse_completion_context(main_module, [current | rest], current_token, command_path, arg_index, waiting_for_option) do
    cond do
      waiting_for_option != nil ->
        ...
        if String.starts_with?(current, "-") do
          ...
          parse_completion_context(main_module, [current | rest], current_token, command_path, arg_index, nil)
        else
      ...
      String.starts_with?(current, "--") ->
        ...
        if String.contains?(option_name, "=") do
          ...
          parse_completion_context(main_module, rest, current_token, command_path, arg_index, option_name)
        end
      ...
      true ->
        ...
        new_arg_index = case Introspection.resolve_command_path(main_module, reversed_new_path) do
          nil -> arg_index  # Invalid command path, don't increment
          ^main_module -> arg_index  # Still at main level
          _command_module -> arg_index + 1  # Complete command, increment for arguments
        end
        
        parse_completion_context(main_module, rest, current_token, new_command_path, new_arg_index, nil)
    end
  end
